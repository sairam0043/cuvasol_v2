...
import { 
  Brain, ShieldAlert, Users, GraduationCap, BookOpen, 
  Briefcase, MessageSquare, LogOut, Menu, X, ArrowLeft 
} from 'lucide-react';
...
export const AdminLayout: React.FC<{ 
  ...
  const menuItems = [
    { id: 'overview', name: 'Analytics Dashboard', icon: ShieldAlert },
    { id: 'users', name: 'User Management', icon: Users },
    { id: 'programs', name: 'Programs Management', icon: GraduationCap },
    { id: 'blogs', name: 'Blog Management', icon: BookOpen },
    { id: 'applications', name: 'Hiring Management', icon: Briefcase },
    { id: 'testimonials', name: 'Testimonials Settings', icon: MessageSquare },
  ];
          ...
          <div className="h-16 flex items-center justify-between px-6 border-b border-zinc-800">
            <Link to="/" className="flex items-center space-x-2.5 group">
              ...
              <span className="font-heading text-base font-bold tracking-tight text-white group-hover:opacity-90 transition-colors">
                ...
                <span className="text-[10px] font-normal text-zinc-450 block -mt-1 leading-none">Admin Control</span>
              </span>
            </Link>
            <button 
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-sm font-medium transition-all cursor-pointer ${
                    isActive
                      ? 'bg-purple-700 text-white shadow-lg shadow-purple-650/15 border-l-4 border-purple-400'
        ...
        <div className="p-4 border-t border-zinc-800 space-y-3">
          <div className="flex items-center space-x-3 px-2">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] text-purple-400 font-medium truncate">System Administrator</p>
            </div>
          </div>
        ...
        <header className="h-16 border-b border-zinc-850 flex items-center justify-between px-6 bg-zinc-950 sticky top-0 z-30">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-base font-bold text-white font-heading tracking-tight capitalize">
              {activeTab === 'overview' ? 'Analytics Cockpit' : activeTab.replace('-', ' ')}
            </h1>
          </div>
          <div className="flex items-center space-x-4">
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-500/10 text-purple-400 border border-purple-500/20">
              <span className="w-1.5 h-1.5 bg-purple-450 rounded-full mr-1.5 animate-pulse" />
              Root Access Active
            </span>
          </div>
        </header>
